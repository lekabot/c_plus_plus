...
TEST(ArrayTest, DefaultConstructor) {
  ...
  EXPECT_EQ(arr.size(), static_cast<size_t>(5));
...
TEST(ArrayTest, CopyConstructor) {
  ...
  // Check that the size is correct
  EXPECT_EQ(arrCopy.size(), static_cast<size_t>(5));
...
TEST(ArrayTest, MoveConstructor) {
  ...
  // Check that the size is correct
  EXPECT_EQ(arrMoved.size(), static_cast<size_t>(5));
...
TEST(ArrayTest, MaxSize) {
  ...
  // size
  EXPECT_EQ(arr.max_size(), static_cast<size_t>(5));
}
