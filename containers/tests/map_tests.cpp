...
TEST(map, MapCapacity) {
  ...
  EXPECT_EQ(my_map.size(), static_cast<size_t>(1));
}
...
TEST(map, SwapMap) {
  ...
  EXPECT_EQ(my_map.size(), static_cast<size_t>(2));
  EXPECT_EQ(my_swap_map.size(), static_cast<size_t>(1));
...
TEST(MapInsertManyTest, InsertMany) {
  ...
  result = m.insert_many(std::make_pair(1, "one"), std::make_pair(2, "two"),
                         std::make_pair(3, "three"));
