...
TEST(VectorTest, DefaultConstructor_EmptyVector) {
  s21::vector<int> v;
  EXPECT_TRUE(v.empty());
  EXPECT_EQ(v.size(), static_cast<size_t>(0));
}
...
TEST(VectorTest, CopyConstructor_EmptyVector) {
  ...
  EXPECT_EQ(v2.size(), static_cast<size_t>(0));
}
...
TEST(VectorTest, MoveConstructor_NonEmptyVector) {
  s21::vector<int> v1 = {1, 2, 3};
  s21::vector<int> v2(std::move(v1));
  EXPECT_EQ(v2.size(), static_cast<size_t>(3));
...
TEST(VectorTest, MaxSize) {
  const s21::vector<int> v;
  EXPECT_GT(v.max_size(), static_cast<size_t>(0));
}
...
TEST(VectorTest, Capacity_EmptyVector) {
  ...
  EXPECT_GE(v.capacity(), static_cast<size_t>(0));
}
...
TEST(VectorTest, PopBack_NonEmptyVector) {
  s21::vector<int> v = {1, 2, 3};
  v.pop_back();
  EXPECT_EQ(v.size(), static_cast<size_t>(2));
  EXPECT_EQ(v.back(), 2);
}
...
TEST(VectorTest, Swap_NonEmptyVectors) {
  ...
  EXPECT_EQ(v1.size(), static_cast<size_t>(3));
  EXPECT_EQ(v2.size(), static_cast<size_t>(3));
...
TEST(VectorInsertManyTest, InsertMany) {
  s21::vector<int> our_vector = {1, 2, 3, 4, 5};
  s21::vector<int>::iterator our_it = our_vector.begin();
  ++our_it;
  our_vector.insert_many(our_it, 7, 8, 9);
  our_it = our_vector.begin();
  EXPECT_EQ(*our_it, 1);
  ++our_it;
  EXPECT_EQ(*our_it, 7);
  ++our_it;
...
TEST(VectorInsertManyTest, InsertManyBack) {
  ...
  EXPECT_EQ(v.size(), static_cast<size_t>(6));
