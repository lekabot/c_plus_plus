...
tree<Key, Value>::TreeNode::TreeNode(Key key, Value valueType)
    : m_Key(key), m_Value(valueType) {}

template <class Key, class Value>
tree<Key, Value>::TreeNode::TreeNode(Key key, Value valueType, TreeNode *parent)
    : m_Key(key), m_Value(valueType), m_Parent(parent) {}
...
typename tree<Key, Value>::TreeNode *tree<Key, Value>::copyTree(
    tree::TreeNode *node, tree::TreeNode *parent) {
...
template <class Key, class Value>
typename tree<Key, Value>::TreeNode *tree<Key, Value>::getMax(
    tree::TreeNode *node) {
  if (node == nullptr) return nullptr;
...
template <class Key, class Value>
typename tree<Key, Value>::TreeNode *tree<Key, Value>::getMin(
    tree::TreeNode *node) {
  if (node == nullptr) return nullptr;
...
void tree<Key, Value>::autoSetHeight(tree::TreeNode *node) {
  node->m_Height =
      std::max(getHeight(node->m_Left), getHeight(node->m_Right)) + 1;
}

template <class Key, class Value>
int tree<Key, Value>::getBalance(tree::TreeNode *node) {
  return node == nullptr ? 0
                         : getHeight(node->m_Right) - getHeight(node->m_Left);
}
...
template <class Key, class Value>
typename tree<Key, Value>::size_type tree<Key, Value>::recursiveSize(
    tree::TreeNode *node) {
...
template <class Key, class Value>
typename tree<Key, Value>::TreeNode *tree<Key, Value>::binarySearch(
    tree::TreeNode *node, const Key &key) {
...
template <class Key, class Value>
bool tree<Key, Value>::recursiveInsert(tree::TreeNode *node, const Key &key,
                                       Value value) {
...
template <class Key, class Value>
typename tree<Key, Value>::TreeNode *tree<Key, Value>::recursiveDelete(
    tree::TreeNode *node, tree::key_type key) {
  if (node == nullptr) return nullptr;
  if (key < node->m_Key) {
...
typename tree<Key, Value>::size_type tree<Key, Value>::max_size() {
  return std::numeric_limits<size_type>::max() /
         sizeof(typename tree<Key, Value>::TreeNode);
}
...
tree<Key, Value>::Iterator::Iterator()
    : m_Iter_Node(nullptr), m_Iter_Past_Node(nullptr) {}

template <class Key, class Value>
tree<Key, Value>::Iterator::Iterator(tree::TreeNode *node,
                                     tree::TreeNode *past_node)
    : m_Iter_Node(node), m_Iter_Past_Node(past_node) {}

template <class Key, class Value>
typename tree<Key, Value>::TreeNode *tree<Key, Value>::Iterator::moveFront(
    tree::TreeNode *node) {
  if (node->m_Right != nullptr) {
...
template <class Key, class Value>
typename tree<Key, Value>::TreeNode *tree<Key, Value>::Iterator::moveBack(
    tree::TreeNode *node) {
  if (node == nullptr) return nullptr;
  if (node->m_Left != nullptr) {
...
template <class Key, class Value>
tree<Key, Value> &tree<Key, Value>::operator=(
    tree<Key, Value> &&other) noexcept {
  if (&other != nullptr) {
