...
typename vector<T>::iterator vector<T>::insert(vector::iterator pos,
  ...
  for (auto i = static_cast<size_type>(m_Size);
       i > static_cast<size_type>(index); --i) {
    m_Data[i] = m_Data[i - 1];
  }
  m_Data[index] = value;
  ++m_Size;
...
typename vector<T>::iterator vector<T>::insert_many(const_iterator pos,
                                                    Args &&...args) {
  size_type offset = pos - cbegin();  // Calculate the offset from the beginning
  size_type count = sizeof...(args);  // Get the number of arguments

  // Check if there's enough capacity to insert the new elements
  if (m_Size + count > m_Capacity) {
    size_type newCapacity = std::max(m_Size + count, m_Capacity * 2);
    ReAlloc(newCapacity);
  }

  // Move elements to make space for new ones
  std::move_backward(begin() + offset, end(), end() + count);

  // Insert the new elements
  size_type index = 0;
  ((m_Data[offset + index++] = std::forward<Args>(args)), ...);
