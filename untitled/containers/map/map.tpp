namespace s21 {
    ...
    template <class Key, class T>
    std::pair<typename map<Key, T>::iterator, bool> map<Key, T>::insert(const map::value_type &value) {
    ...
    std::pair<typename map<Key, T>::iterator, bool> map<Key, T>::insert(const Key &key, const T &obj) {
        ...
        if (tree<Key, T>::m_Root == nullptr) {
            ...
            bool inserted = tree<Key, T>::recursiveInsert(tree<Key, T>::m_Root, key, obj);
    ...
    typename map<Key, T>::value_type& map<Key, T>::iterator::operator*() {
        ...
        std::pair<const key_type, mapped_type> pr =
                std::make_pair(tree<Key, T>::iterator::m_Iter_Node->m_Key,
    ...
    T &map<Key, T>::operator[](const Key &key) {
        auto it = find(key);
        if (it == nullptr) {
            std::pair<typename map<Key, T>::iterator, bool> pr = insert(std::make_pair(key, T()));
            it = pr.first;
        }
    ...
    void map<Key, T>::erase(map::iterator pos) {
        ...
        tree<Key, T>::m_Root = tree<Key, T>::recursiveDelete(tree<Key, T>::m_Root, (*pos).first);
    }
