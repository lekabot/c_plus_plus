...
TEST(map, MergeMap) {
    ...
    s21::map<int, int> my_map_merge = {{3, 3}, {4, 5}};

    std::map<int, int> orig_map = {{1, 1}, {4, 4}, {2, 2}};
    std::map<int, int> orig_map_merge = {{3, 3}, {4, 5}};

    my_map.merge(my_map_merge);
    orig_map.merge(orig_map_merge);

    for (const auto& [key, value] : my_map_merge) {
        EXPECT_TRUE(my_map.contains(key));
        EXPECT_EQ(my_map[key], value);
    }
